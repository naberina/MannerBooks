...
#### 1. 関数宣言
関数宣言も、変数宣言(`const`, `let`,  `var`)と同様に、「ここから関数を定義します」の意味を持つ `function` に続けて関数を定義します。関数名は、処理内容が分かりやすい名前をつけましょう。

#### 2.パラメータ
パラメータとは、関数の定義内の変数のことです。
...
### 関数の呼び出し方
パラメータなしの場合は、 `関数名();`
パラメータあり場合は、 `関数名(引数);` で関数を呼び出すことができます。
...
引数を使用すると、関数の処理部分に値を渡すことができるため、条件分岐を用いて、引数によって異なる戻り値を返すこともできます。
...
関数宣言の前に関数を呼び出しても実行できる理由は、「[関数宣言の巻き上げ](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Statements/function#function_declaration_hoisting)」が起きるためです。つまり、関数宣言は書いてあるコード順を守らなくても実行できてしまい、コードが複雑になる可能性が高くなります。
...
しかし、よく見ると先ほどの関数宣言とは異なり、`function`の後の関数名が省略されています。これは無名関数と呼ばれるものです。
...
また、関数式は代入処理のタイミングで関数が定義されるため、巻き上げ処理が起こりません。そのため、関数宣言後に関数を呼び出すことができるため、コードの記述順に沿った動作になります。特別な理由がない場合は、関数式で関数を定義する方が好ましいです。
